		...
		if(true){
        	...
        	if (seoDescription.length<5 || seoDescription.length>500){$('cseoDescription').innerHTML = '<font color=red>*描述  输入不符合规范,必须在5到500个字符之间</font>';
        	  return false;
      		 ...
      		 return false;
      		 }else{ $('cpicContent').innerHTML = "&nbsp;&nbsp;<img src='/resources/mfolder/images/reg4.gif' />";  			  }
 		}
...
<body>
   ...
   <sf:form id="myform" method="post" action="/masteLo/adpic/add.html" modelAttribute="adpic" onsubmit="return addmessFun();" enctype="multipart/form-data">
    <ul class="forminfo">
	    ...
	    <li><label>广告标题</label><input id="picname" onblur="caddmessFun('picname');" name="picname" type="text" class="dfinput" value="" /><i id="cpicname">* 信息标题 ，必须在5到50个字符之间</i></li>
	    <li><label>后台关键字</label><input id="classkey" onblur="caddmessFun('classkey');" name="classkey" type="text" class="dfinput" value="" /><i id="cclasskey">* 请输入后台关键字eg: banner(首页-圆)、tvpic(首页-视频图) </i></li>
	    <li><label>广告链接</label><input id="picurl" onblur="caddmessFun('picurl');" name="picurl" type="text" class="dfinput" value="http://" /><i id="cpicurl">* 请输入广告链接地址</i></li>
	    <li><label>编辑者</label><input id="author" onblur="caddmessFun('author');" name="author" type="text" class="dfinput" value="" /><i id="cauthor">* 请输入编辑者</i></li>
	    <li><label>排序[升序]</label><input id="picOrder" onblur="caddmessFun('picOrder');" name="picOrder" type="text" class="dfinput" value="" /><i id="cpicOrder">* 请输入排序ID 越小越靠前</i></li>
	    <li><label>图片地址</label><input id="picImg" onblur="caddmessFun('picImg');" name="picImg" readonly type="text" class="dfinput" value="ddd" /><input type="button" value=" 图片添加 " name="fckpic1" class="btnpic" onclick="openUpladage('picImg','pic','ad');" /><i id="cpicImg">* 请上传图片</i></li>
	    <li><label>显示时间</label><input id="picstartime" onblur="caddmessFun('picstartime');" class="Wdate" onclick="WdatePicker()" readonly name="picstartime" type="text" class="dfinput" value="" /><i id="cpicstartime">* 请选择广告开始时间</i></li>
	    <li><label>结束时间</label><input id="picendtime" onblur="caddmessFun('picendtime');"  class="Wdate" onclick="WdatePicker()" readonly name="picendtime" type="text" class="dfinput" value="" /><i id="cpicendtime">* 请选择广告结束时间</i></li>
	    <li><label>推荐</label>
	   ...
	   <i id="cisRecommend">* 默认推荐</i></li>
	    ...
	    <textarea name="picContent" id="tscompanyContent" rows="8" style="display:none"></textarea>
		           <iframe id="FCKeditor1___Frame" src="/resources/ceditor/editor/fckeditor.htm?InstanceName=tscompanyContent&amp;Toolbar=Default" width="100%" height="350" frameborder="0" scrolling="no"></iframe><font style="color:#7f7f7f;" id="cpicContent">&nbsp;&nbsp;* 资料内容 </font></li>
