		...
		if(true){
        	...
        	if (seoDescription.length<5 || seoDescription.length>500){$('cseoDescription').innerHTML = '<font color=red>*描述  输入不符合规范,必须在5到500个字符之间</font>';
        	  return false;
      		 ...
      		 return false;
      		 }else{ $('ctscompanyContent').innerHTML = "&nbsp;&nbsp;<img src='/mfolder/images/reg4.gif' />";  			  }
 		}
...
<body>
   ...
   <form id="myform" name="form1" method="post" action="/mastLo/website_pedtfun.html" onsubmit="return addmessFun();">
    <ul class="forminfo">
	    <li><label>关键字</label><input type="hidden" name="adpic.id" value='<s:property value ="adpic.id" />'/><textarea id="seoKeyword" onblur="caddmessFun('seoKeyword');" name="adpic.seoKeyword" cols="" rows="" class="textinput"><s:property value="adpic.seoKeyword" /></textarea><i id="cseoKeyword">* 关键字 必须在 5到500个字符之间</i></li>
	    <li><label>描述</label><textarea id="seoDescription" onblur="caddmessFun('seoDescription');" name="adpic.seoDescription" cols="" rows="" class="textinput"><s:property value="adpic.seoDescription" /></textarea><i id="cseoDescription">描述必须在5到500个字符之间</i></li>	   
	    <li><label>广告标题</label><input id="picname" onblur="caddmessFun('picname');" name="adpic.picname" type="text" class="dfinput" value="<s:property value="adpic.picname"/>" /><i id="cpicname">* 信息标题 ，必须在5到50个字符之间</i></li>
	    <li><label>后台关键字</label><input id="classkey" onblur="caddmessFun('classkey');" name="adpic.classkey" type="text" class="dfinput" value="<s:property value="adpic.classkey"/>" /><i id="cclasskey">* 请输入后台关键字eg: banner </i></li>
	    <li><label>广告链接</label><input id="picurl" onblur="caddmessFun('picurl');" name="adpic.picurl" type="text" class="dfinput" value="<s:property value="adpic.picurl"/>" /><i id="cpicurl">* 请输入广告链接地址</i></li>
	    <li><label>编辑者</label><input id="author" onblur="caddmessFun('author');" name="adpic.author" type="text" class="dfinput" value="<s:property value="adpic.author" />" /><i id="cauthor">* 请输入编辑者</i></li>
	    <li><label>排序[升序]</label><input id="picOrder" onblur="caddmessFun('picOrder');" name="adpic.picOrder" type="text" class="dfinput" value="<s:property value="adpic.picOrder" />" /><i id="cpicOrder">* 请输入排序ID 越小越靠前</i></li>
	    <li><label>图片地址</label><input id="picImg" onblur="caddmessFun('picImg');" name="adpic.picImg" readonly type="text" class="dfinput" value="<s:property value="adpic.picImg" />" /><input type="button" value=" 图片添加 " name="fckpic1" class="btnpic" onclick="openUpladage('picImg','pic','ad');" /><i id="cpicImg">* 请上传图片</i></li>
	    ...
	    <select name="adpic.isRecommend">
	    	<option <s:if test="1==adpic.isRecommend">selected</s:if> value='1'>&nbsp;&nbsp;推荐&nbsp;&nbsp;</option>
	    	<option <s:if test="0==adpic.isRecommend">selected</s:if> value='0'>&nbsp;&nbsp;否&nbsp;</option>
	    </select>
	    <i id="cisRecommend">* 默认推荐</i></li>
	    <li><label>状态</label>
	    <select name="adpic.picState">
	    	<option <s:if test="1==adpic.picState">selected</s:if> value='1'>&nbsp;&nbsp;正常&nbsp;&nbsp;</option>
	    	<option <s:if test="0==adpic.picState">selected</s:if> value='0'>&nbsp;&nbsp;关闭&nbsp;</option>
	    </select>
	    ...
	    <textarea name="adpic.picContent" id="tscompanyContent" rows="8" style="display:none"><s:property value="adpic.picContent" /></textarea>
		           <iframe id="FCKeditor1___Frame" src="/Editor/editor/fckeditor.htm?InstanceName=tscompanyContent&amp;Toolbar=Default" width="100%" height="350" frameborder="0" scrolling="no"></iframe><font style="color:#7f7f7f;" id="ctscompanyContent">&nbsp;&nbsp;* 资料内容 </font></li>
