...
<body>
   ...
   <form id="myform" name="form1" method="post" action="/mastLo/pro_edtfun.html" onsubmit="return aaddmessFun();">
    <ul class="forminfo">
	    <li><label>关键字</label><input type="hidden" name="cpro.id" value='<s:property value ="cpro.id" />'/><textarea id="seoKeyword" onblur="caddmessFun('seoKeyword');" name="cpro.seoKeyword" cols="" rows="" class="textinput"><s:property value="cpro.seoKeyword" /></textarea><i id="cseoKeyword">* 关键字 必须在 5到500个字符之间</i></li>
	    <li><label>seo描述</label><textarea id="seoDescription" onblur="caddmessFun('seoDescription');" name="cpro.seoDescription" cols="" rows="" class="textinput"><s:property value="cpro.seoDescription" /></textarea><i id="cseoDescription">SEO描述必须在5到500个字符之间</i></li>
	    <li><label>产品描述</label><textarea id="proCaption" onblur="caddmessFun('proCaption');" name="cpro.proCaption" cols="" rows="" class="textinput"><s:property value="cpro.proCaption" /></textarea><i id="cproCaption">产品描述必须在5到500个字符之间</i></li>	   
	    <li><label>标题</label><input id="productName" onblur="caddmessFun('productName');" name="cpro.productName" type="text" class="dfinput" value="<s:property value="cpro.productName"/>" /><i id="cproductName">* 信息标题 ，必须在2到20个字符之间</i></li>
	    ...
	    <li><label>产品图片</label><input id="proImg" onblur="caddmessFun('proImg');" name="cpro.proImg" readonly type="text" class="dfinput" value="<s:property value ="cpro.proImg" />" /><input type="button" value=" 图片添加 " name="fckpic1" class="btnpic" onclick="openUpladage('proImg','pic','cp');" /><i id="cproImg">* 请上传产品图片</i></li>
	    <li><label>发布者</label><input id="author" onblur="caddmessFun('author');" name="cpro.author" type="text" class="dfinput" value="<s:property value ="cpro.author" />" /><i id="cauthor">* 请填写编辑者,必须在2到5个字符之间</i></li>
	    <li><label>点击次数</label><input id="looknum" onblur="caddmessFun('looknum');" name="cpro.looknum" type="text" class="dfinput" value="<s:property value ="cpro.looknum" />" /><i id="clooknum">* 请输入点击次数 默认1</i></li>
	    <li><label>是否推荐</label>
	    <select name="cpro.isRecommend">
	    	<option <s:if test="1==cpro.isRecommend">selected</s:if> value='1'>&nbsp;&nbsp;推荐&nbsp;&nbsp;</option>
	    	<option <s:if test="0==cpro.isRecommend">selected</s:if> value='0'>&nbsp;&nbsp;否&nbsp;</option>
	    ...
	    <textarea name="cpro.proContent" id="tscompanyContent" rows="8" style="display:none"><s:property value ="cpro.proContent" /></textarea>
